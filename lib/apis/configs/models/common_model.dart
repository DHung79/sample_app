class Paging {
  final int _totalDocuments;
  final int _limit;
  final int _page;
  final int _totalPage;

  Paging.fromJson(Map<String, dynamic> json)
      : _totalDocuments = json['totalDocuments'] ?? 0,
        _totalPage = json['totalPages'] ?? 0,
        _limit = int.tryParse(json['limit']?.toString() ?? '0') ?? 0,
        _page = int.tryParse(json['page']?.toString() ?? '0') ?? 0;

  Paging.fromGraphqlJson(Map<String, dynamic> json)
      : _totalDocuments = json['total'] ?? 0,
        _totalPage = json['lastPage'] ?? 0,
        _limit = int.tryParse(json['perPage']?.toString() ?? '0') ?? 0,
        _page = int.tryParse(json['currentPage']?.toString() ?? '0') ?? 0;

  int get limit => _limit;
  int get page => _page;
  int get totalPage => _totalPage;
  int get totalRecords => _totalDocuments;
